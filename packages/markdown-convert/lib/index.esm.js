import { Modifier, EditorState, SelectionState, AtomicBlockUtils } from 'draft-js';
...
function changeInlineStyle(editorState, text, inlineStyle, markFlag) {
  ...
  contentState = Modifier.replaceText(contentState, contentState.getSelectionAfter().merge({
  ...
  for (var _i = 0, inlineStyle_1 = inlineStyle; _i < inlineStyle_1.length; _i++) {
    ...
    contentState = Modifier.applyInlineStyle(contentState, contentState.getSelectionAfter().merge({
      anchorOffset: startIndex
    }), style);
  }

  var newEditorState = EditorState.push(editorState, // force the focus offset to the end
  ...
  for (var _a = 0, inlineStyle_2 = inlineStyle; _a < inlineStyle_2.length; _a++) {
    ...
    newEditorState = EditorState.setInlineStyleOverride(newEditorState, newEditorState.getCurrentInlineStyle().delete(style));
  }
...
function changeBlockType(editorState, blockType) {
  ...
  contentState = Modifier.setBlockType(contentState, SelectionState.createEmpty(focusKey).merge({
    hasFocus: true
  }), blockType);
...
function handleReturnBlock(editorState, block, entity) {
  ...
  var newEditorState = EditorState.push(editorState, contentState, 'remove-range');
  ...
  if (blockType === 'code-block') {
    ...
    if (blockData) {
      contentState = Modifier.mergeBlockData(contentState, SelectionState.createEmpty(focusKey).merge({
        hasFocus: true
      }), blockData);
    }
    ...
    if (entityType) {
      ...
      newEditorState = AtomicBlockUtils.insertAtomicBlock(newEditorState, contentState.getLastCreatedEntityKey(), ' ');
    }
  }
  ...
  return {
    handleBeforeInput: function (chars, editorState, eventTimeStamp, _a) {
      ...
      if (editorState.getSelection().isCollapsed() && [' ', '*', '_', '~', '`'].includes(chars)) {
        var contentStateWithChars = Modifier.insertText(editorState.getCurrentContent(), editorState.getSelection(), chars, editorState.getCurrentInlineStyle());
        var newEditorState = EditorState.forceSelection(EditorState.push(editorState, contentStateWithChars, 'insert-characters'), contentStateWithChars.getSelectionAfter());
...
export { createConvertPlugin as default };
